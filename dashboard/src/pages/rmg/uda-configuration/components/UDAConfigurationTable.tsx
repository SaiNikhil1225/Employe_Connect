...
export function UDAConfigurationTable({
  ...
  // Group configurations by billable status and sort in ascending order
  const groupedConfigurations = useMemo(() => {
    ...
    // Group configurations by billable status
    const billable = configurations
      .filter((c) => c.billable === 'Billable')
      .sort(sortByUDANumber);
    
    const nonBillable = configurations
      .filter((c) => c.billable === 'Non-Billable')
      .sort(sortByUDANumber);
