  ...
  const parseExcelFile = async (file: File) => {
    ...
    try {
      ...
      (jsonData as Record<string, unknown>[]).forEach((row, index: number) => {
        ...
        const previousCTCRaw = row.PreviousCTC || row.previousCTC || row.previous_ctc || row.PREVIOUSCTC || '';
        const currentCTCRaw = row.CurrentCTC || row.currentCTC || row.current_ctc || row.CURRENTCTC || '';
        const previousCTC = previousCTCRaw !== '' ? parseFloat(String(previousCTCRaw)) : undefined;
        const currentCTC = currentCTCRaw !== '' ? parseFloat(String(currentCTCRaw)) : undefined;
        ...
        employees.push({
          ...
          ...(previousCTC !== undefined && !isNaN(previousCTC) ? { previousCTC } : {}),
          ...(currentCTC !== undefined && !isNaN(currentCTC) ? { currentCTC } : {}),
        });
      });
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="border rounded-lg overflow-hidden">
            <div className="bg-muted/60 px-4 py-2 border-b">
              <p className="text-sm font-medium">Supported Columns</p>
              <p className="text-xs text-muted-foreground">Columns marked * are required</p>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-1.5 px-4 py-3 text-xs">
              {[
                { col: 'EmployeeID', req: true },
                { col: 'Name', req: true },
                { col: 'Email', req: true },
                { col: 'Department', req: true },
                { col: 'Designation', req: false },
                { col: 'Location', req: false },
                { col: 'DateOfJoining', req: false },
                { col: 'BusinessUnit', req: false },
                { col: 'ReportingManagerId', req: false },
                { col: 'Phone', req: false },
                { col: 'Status', req: false },
                { col: 'DateOfBirth', req: false },
                { col: 'PreviousCTC', req: false },
                { col: 'CurrentCTC', req: false },
              ].map(({ col, req }) => (
                <div key={col} className="flex items-center gap-1.5">
                  <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${req ? 'bg-red-500' : 'bg-muted-foreground/40'}`} />
                  <span className={req ? 'font-semibold text-foreground' : 'text-muted-foreground'}>{col}{req ? ' *' : ''}</span>
                </div>
              ))}
            </div>
          </div>
                    ...
                    <table className="w-full text-sm">
                      <thead className="bg-green-100 sticky top-0">
                        <tr className="text-left text-green-900">
                          ...
                          <th className="px-2 py-1">Prev CTC</th>
                          <th className="px-2 py-1">Curr CTC</th>
                        </tr>
                      </thead>
                      <tbody className="text-green-800">
                        {parsedData.slice(0, 10).map((emp, index) => (
                          <tr key={index} className="border-t border-green-200">
                            ...
                            <td className="px-2 py-1">{emp.previousCTC != null ? `₹${emp.previousCTC.toLocaleString()}` : '—'}</td>
                            <td className="px-2 py-1">{emp.currentCTC != null ? `₹${emp.currentCTC.toLocaleString()}` : '—'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
