...
interface EmployeeStore {
  ...
  getNextEmployeeId: (workerType?: string) => Promise<string>;
  addEmployee: (employee: Omit<Employee, '_id'>) => Promise<any>;
...
export const useEmployeeStore = create<EmployeeStore>((set, get) => ({
  ...
  getNextEmployeeId: async (workerType?: string) => {
    try {
      return await employeeService.getNextEmployeeId(workerType);
    } catch {
      // Generate local ID as fallback based on worker type
      const employees = get().employees;
      let prefix = 'ACUA'; // Default for Full-Time/Part-Time
      ...
      employees.forEach(emp => {
        if (emp.employeeId.startsWith(prefix)) {
          const num = parseInt(emp.employeeId.replace(prefix, ''));
  ...
  addEmployee: async (employee) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 'Failed to add employee. Please check if the server is running.';
