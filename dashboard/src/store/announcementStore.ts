...
export interface Announcement {
  ...
  firstReactedBy?: string;
  firstReactedAt?: string;
  latestReactedBy?: string;
  latestReactedAt?: string;
  firstCommentedBy?: string;
  firstCommentedAt?: string;
  status?: 'draft' | 'published' | 'scheduled' | 'archived';
  expiresAt?: string; // Expiry date for announcements
  scheduledAt?: string; // Date/time to publish when status is 'scheduled'
  targetDepartments?: string[]; // Dept names if audience is 'departments'
  notifyByEmail?: boolean; // Whether to notify employees via email
...
interface AnnouncementState {
  ...
  updateAnnouncement: (id: number | string, updates: Partial<Announcement>) => Promise<void>;
  deleteAnnouncement: (id: number | string) => Promise<void>;
  toggleLike: (id: number | string, userId: string, userDetails?: { userName: string; department: string; role: string }) => Promise<void>;
  addReaction: (id: number | string, oderId: string, userName: string, emoji: string, label: string) => Promise<void>;
  addComment: (id: number | string, comment: Omit<Comment, 'id'>) => Promise<void>;
  votePoll: (id: number | string, optionIds: string[], oderId: string) => Promise<void>;
}
...
export const useAnnouncementStore = create<AnnouncementState>((set, get) => ({
      ...
      if (result.success) {
        ...
        set({
          announcements: get().announcements.map(a =>
            (a.id === id || a.id === result.data._id) ? updatedAnnouncement : a
          ),
        });
      }
    } catch (_error) {
  ...
  toggleLike: async (id, userId, userDetails) => {
    try {
