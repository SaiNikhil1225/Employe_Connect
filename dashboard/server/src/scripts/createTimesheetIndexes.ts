...
async function createTimesheetIndexes() {
    try {
        ...
        await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/employee_connect');
        ...
        await timesheetEntriesCollection.createIndex(
            { employeeId: 1, date: 1 },
        ...
        await timesheetEntriesCollection.createIndex(
            { approvalStatus: 1, submittedAt: 1 },
        ...
        await timesheetsCollection.createIndex(
            { employeeId: 1, weekStartDate: 1 },
            { name: 'idx_employee_week' }
        );
