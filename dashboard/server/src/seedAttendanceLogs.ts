...
import { format, subDays, addDays, startOfDay, parse, differenceInMinutes, isWeekend } from 'date-fns';
...
const getRandomStatus = (): 'present' | 'absent' | 'late' | 'wfh' | 'leave' | 'half-day' => {
  const rand = Math.random();
  if (rand < 0.70) return 'present'; // 70% present
  if (rand < 0.75) return 'late';    // 5% late
  if (rand < 0.80) return 'wfh';     // 5% wfh
  if (rand < 0.88) return 'absent';  // 8% absent
  if (rand < 0.95) return 'leave';   // 7% leave
  return 'half-day';                  // 5% half-day
};

// Generate check-in time based on status
const generateCheckInTime = (date: Date, status: string, graceMinutes: number): Date | undefined => {
  if (status === 'absent' || status === 'leave' || status === 'weekly-off') return undefined;
...
const generateCheckOutTime = (checkIn: Date | undefined, status: string): Date | undefined => {
  if (!checkIn || status === 'absent' || status === 'leave' || status === 'weekly-off') return undefined;
  ...
  try {
    ...
    while (currentDate <= today) {
      ...
      for (const employee of employees) {
        let status: 'present' | 'absent' | 'late' | 'wfh' | 'leave' | 'weekly-off' | 'half-day';
        ...
        if (checkInTime && status === 'present') {
          const standardTime = parse(policy.standardCheckIn, 'h:mm a', dateStart);
    ...
    const totalRecords = await AttendanceLog.countDocuments();
    const presentCount = await AttendanceLog.countDocuments({ status: 'present' });
    const lateCount = await AttendanceLog.countDocuments({ status: 'late' });
    const absentCount = await AttendanceLog.countDocuments({ status: 'absent' });
    const wfhCount = await AttendanceLog.countDocuments({ status: 'wfh' });
    const leaveCount = await AttendanceLog.countDocuments({ status: 'leave' });
    const weeklyOffCount = await AttendanceLog.countDocuments({ status: 'weekly-off' });
