...
export const commonValidations = {
  ...
  employeeIdOrMongoId: param('id')
    .custom((value) => {
      // Accept either MongoDB ObjectId or custom employee ID format
      const isMongoId = /^[0-9a-fA-F]{24}$/.test(value);
      const isCustomEmployeeId = /^[A-Z]{2,4}\d{3,5}$/.test(value); // Matches HR001, HM001, ACUA0001, etc.
      return isMongoId || isCustomEmployeeId;
    })
...
export const employeeValidation = {
  ...
  update: [
    commonValidations.employeeIdOrMongoId,
    body('name')
      .optional()
      .trim()
      .isLength({ min: 2, max: 100 })
