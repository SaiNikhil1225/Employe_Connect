...
router.get('/unread/count', async (req: Request, res: Response) => {
  try {
    ...
    // Match notifications sent to this specific user via recipientIds
    if (userId) {
      orConditions.push({ recipientIds: userId as string });
    }
    
    // Legacy: Match notifications sent to this specific user via userId
    if (userId) {
      orConditions.push({ userId: userId as string });
    }
    
    // Legacy: Match notifications sent to this role (for approvers, IT admins, etc.)
    if (role && role !== 'all') {
      orConditions.push({ role: role as string });
    }
    
    // Legacy: Always include broadcast notifications
