  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!settings) {
      settings = await CompanySettings.create({
        ...
        designations: [],
        businessUnits: [],
        updatedBy: req.user?.employeeId || 'SYSTEM'
      });
    }
...
/**
 * PUT /superadmin/company-settings
 * Update company settings
 */
router.put(
  '/company-settings',
  asyncHandler(async (req: Request, res: Response) => {
    const { legalEntities, locations, taxIdConfigs, departments, designations, businessUnits } = req.body;
    ...
    if (!settings) {
      ...
      if (legalEntities) settings.legalEntities = legalEntities;
      if (locations) settings.locations = locations;
      if (taxIdConfigs) settings.taxIdConfigs = taxIdConfigs;
      if (departments) settings.departments = departments;
      if (designations) settings.designations = designations;
      if (businessUnits) settings.businessUnits = businessUnits;
      settings.lastUpdated = new Date();
      settings.updatedBy = req.user?.employeeId || 'SYSTEM';
    }
...
/**
 * POST /superadmin/company-settings/legal-entities
 * Add a new legal entity
 */
router.post(
  '/company-settings/legal-entities',
  asyncHandler(async (req: Request, res: Response) => {
    const { name } = req.body;
    ...
    if (!settings) {
      return res.status(404).json({
        success: false,
        message: 'Company settings not found. Please initialize settings first.'
      });
    }

    if (settings.legalEntities.includes(name)) {
      return res.status(400).json({
        success: false,
        message: 'Legal entity already exists'
      });
    }

    settings.legalEntities.push(name);
    settings.lastUpdated = new Date();
    settings.updatedBy = req.user?.employeeId || 'SYSTEM';
    await settings.save();
    ...
    res.json({
      success: true,
      message: 'Legal entity added successfully',
      data: settings.legalEntities
    });
  })
);

/**
 * POST /superadmin/company-settings/locations
 * Add a new location
 */
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!name || !country) {
      return res.status(400).json({
        success: false,
        message: 'Location name and country are required'
      });
    }
    ...
    const locationExists = settings.locations.some(loc => loc.name === name);
    if (locationExists) {
      return res.status(400).json({
        success: false,
    ...
    settings.locations.push({ name, country });
    settings.lastUpdated = new Date();
    settings.updatedBy = req.user?.employeeId || 'SYSTEM';
    await settings.save();
