...
export function EmployeeCard({ person, type, onHover, isHovered }: EmployeeCardProps) {
  ...
  const getAvatarColor = (name: string) => {
    // Generate a consistent color based on the person's name for variety
        ...
        <PopoverTrigger asChild>
          ...
          >
            {person.profilePhoto ? (
              ...
              <div className={`h-12 w-12 rounded-full ${getAvatarColor(person.name)} flex items-center justify-center text-white font-medium text-base flex-shrink-0 shadow-sm transition-transform duration-200 group-hover:scale-105`}>
                {person.avatar}
              </div>
            )}
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 pb-3 border-b">
        {employee.profilePhoto ? (
          ...
          <div className="h-12 w-12 rounded-full bg-gradient-to-br from-primary to-primary/80 flex items-center justify-center text-white font-medium text-base shadow-sm flex-shrink-0">
            {employee.avatar}
          </div>
        )}
        <div className="flex-1 min-w-0">
