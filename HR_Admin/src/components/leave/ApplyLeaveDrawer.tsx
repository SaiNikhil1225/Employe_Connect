...
import { LEAVE_TYPE_ICONS } from '@/types/leave';
...
export function ApplyLeaveDrawer({
  ...
  const leaveTypeAvailability = useMemo(() => {
    if (leaveDays === 0) {
      // When no days selected, all leave types are technically available
      const availability: Record<string, boolean> = {};
      if (leaveBalance?.leaveTypes) {
        leaveBalance.leaveTypes.forEach((lt) => {
          availability[lt.type] = true;
        });
      }
      return availability;
    }

    // Check availability based on new leaveTypes structure
    const availability: Record<string, boolean> = {};
    if (leaveBalance?.leaveTypes) {
      leaveBalance.leaveTypes.forEach((lt) => {
        availability[lt.type] = leaveDays <= (lt.available || 0);
      });
    }
    ...
    if (!leaveBalance?.leaveTypes) {
      return {
        'Earned Leave': leaveDays <= (leaveBalance?.earnedLeave?.remaining ?? 20),
        'Sabbatical Leave': leaveDays <= (leaveBalance?.sabbaticalLeave?.remaining ?? 180),
        'Comp Off': leaveDays <= (leaveBalance?.compOff?.remaining ?? 0),
        'Paternity Leave': leaveDays <= (leaveBalance?.paternityLeave?.remaining ?? 3)
      };
    }
  ...
  const getLeaveBalanceInfo = () => {
    ...
    const colorMap: Record<string, string> = {
      ...
      'Marriage Leave': 'yellow',
      'Loss of Pay': 'slate',
      'Annual Leave': 'indigo'
    };

    // Handle new leaveTypes structure
    if (leaveBalance?.leaveTypes && leaveBalance.leaveTypes.length > 0) {
      const selectedLeaveType = leaveBalance.leaveTypes.find(lt => lt.type === formData.leaveType);
      
      if (selectedLeaveType) {
        const iconName = LEAVE_TYPE_ICONS[selectedLeaveType.type] || 'Circle';
        const IconComponent = iconMap[iconName] || Circle;
        const color = colorMap[selectedLeaveType.type] || 'gray';
        
        return {
          total: selectedLeaveType.allocated || selectedLeaveType.accrued || 0,
          remaining: selectedLeaveType.available || 0,
          color: color,
          icon: IconComponent
        };
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = error?.response?.data?.message || 
                          ...
                          error?.message ||
                          (editData ? 'Failed to update leave request. Please try again.' : 'Failed to submit leave request. Please try again.');
              ...
              >
                ...
                <SelectContent>
                  {leaveBalance?.leaveTypes && leaveBalance.leaveTypes.length > 0 ? (
                    // Dynamic rendering based on employee's leave plan
                    <>
                      {leaveBalance.leaveTypes.map((lt) => {
                        const iconMap: Record<string, any> = {
                          'Palmtree': Palmtree,
                          'Plane': Plane,
                          'Heart': Heart,
                        ...
                        const colorMap: Record<string, string> = {
                          'Casual Leave': 'text-purple-500',
                          'Earned Leave': 'text-blue-500',
                          'Sick Leave': 'text-red-500',
                          'Compensatory Off': 'text-orange-500',
                          'Maternity Leave': 'text-pink-500',
                          'Paternity Leave': 'text-green-500',
                          'Bereavement Leave': 'text-gray-500',
                          'Marriage Leave': 'text-yellow-500',
                          'Loss of Pay': 'text-slate-500',
                          'Annual Leave': 'text-indigo-500'
                        };

                        const iconName = LEAVE_TYPE_ICONS[lt.type] || 'Circle';
                        const IconComponent = iconMap[iconName] || Circle;
                        const color = colorMap[lt.type] || 'text-gray-500';
                        const isAvailable = leaveTypeAvailability[lt.type] !== false;

                        return (
                          <SelectItem key={lt.type} value={lt.type} disabled={!isAvailable}>
                            <div className="flex items-center gap-2">
                              <IconComponent className={cn("h-4 w-4", !isAvailable ? "text-gray-400" : color)} />
                              <div className="flex flex-col">
                                <span className={cn("font-medium", !isAvailable && "text-gray-400")}>{lt.type}</span>
                                <span className="text-xs text-muted-foreground">
                                  {lt.available ?? 0} days available
                                  {!isAvailable && leaveDays > 0 && " (insufficient balance)"}
                                </span>
                              </div>
                            </div>
                          </SelectItem>
                      ...
                      {/* Always show Loss of Pay as fallback */}
                      {!leaveBalance.leaveTypes.some(lt => lt.type === 'Loss of Pay') && (
                        <SelectItem value="Loss of Pay">
                          <div className="flex items-center gap-2">
                            <XCircle className="h-4 w-4 text-red-500" />
                            <div className="flex flex-col">
                              <span className="font-medium">Loss of Pay</span>
                              <span className="text-xs text-muted-foreground">
                                {wouldExceedEarnedLeave ? "Required - Leave balance exceeded" : "Unlimited days available"}
                              </span>
                            </div>
                          </div>
                        </SelectItem>
                    ...
                    // Backwards compatibility with old structure
                    <>
                      <SelectItem value="Earned Leave" disabled={!leaveTypeAvailability['Earned Leave']}>
                        <div className="flex items-center gap-2">
                          <Briefcase className={cn("h-4 w-4", !leaveTypeAvailability['Earned Leave'] ? "text-gray-400" : "text-blue-500")} />
                          <div className="flex flex-col">
                            <span className={cn("font-medium", !leaveTypeAvailability['Earned Leave'] && "text-gray-400")}>Earned Leave</span>
                            <span className="text-xs text-muted-foreground">
                              {leaveBalance?.earnedLeave?.remaining ?? 20} days available
                              {!leaveTypeAvailability['Earned Leave'] && leaveDays > 0 && " (insufficient balance)"}
                            </span>
                          </div>
                        </div>
                      </SelectItem>
                      <SelectItem value="Sabbatical Leave" disabled={!leaveTypeAvailability['Sabbatical Leave']}>
                        <div className="flex items-center gap-2">
                          <Palmtree className={cn("h-4 w-4", !leaveTypeAvailability['Sabbatical Leave'] ? "text-gray-400" : "text-purple-500")} />
                          <div className="flex flex-col">
                            <span className={cn("font-medium", !leaveTypeAvailability['Sabbatical Leave'] && "text-gray-400")}>Sabbatical Leave</span>
                            <span className="text-xs text-muted-foreground">
                              {leaveBalance?.sabbaticalLeave?.remaining ?? 180} days available
                              {!leaveTypeAvailability['Sabbatical Leave'] && leaveDays > 0 && " (insufficient balance)"}
                            </span>
                          </div>
                        </div>
                      </SelectItem>
                      <SelectItem value="Comp Off" disabled={!leaveTypeAvailability['Comp Off']}>
                        <div className="flex items-center gap-2">
                          <Award className={cn("h-4 w-4", !leaveTypeAvailability['Comp Off'] ? "text-gray-400" : "text-orange-500")} />
                          <div className="flex flex-col">
                            <span className={cn("font-medium", !leaveTypeAvailability['Comp Off'] && "text-gray-400")}>Comp Off</span>
                            <span className="text-xs text-muted-foreground">
                              {leaveBalance?.compOff?.remaining ?? 0} days available
                              {!leaveTypeAvailability['Comp Off'] && leaveDays > 0 && " (insufficient balance)"}
                            </span>
                          </div>
                        </div>
                      </SelectItem>
                      <SelectItem value="Paternity Leave" disabled={!leaveTypeAvailability['Paternity Leave']}>
                        <div className="flex items-center gap-2">
                          <Baby className={cn("h-4 w-4", !leaveTypeAvailability['Paternity Leave'] ? "text-gray-400" : "text-green-500")} />
                          <div className="flex flex-col">
                            <span className={cn("font-medium", !leaveTypeAvailability['Paternity Leave'] && "text-gray-400")}>Paternity Leave</span>
                            <span className="text-xs text-muted-foreground">
                              {leaveBalance?.paternityLeave?.remaining ?? 3} days available
                              {!leaveTypeAvailability['Paternity Leave'] && leaveDays > 0 && " (insufficient balance)"}
                            </span>
                          </div>
                        </div>
                      </SelectItem>
