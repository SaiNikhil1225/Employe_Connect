...
export function Dashboard() {
  ...
  // Redirect HR users to Workforce Summary page only in admin mode (not in user mode)
  useEffect(() => {
    if (user?.role === 'HR' && !permissions.isHRUser) {
      navigate('/hr/workforce-summary', { replace: true });
    }
  }, [user?.role, permissions.isHRUser, navigate]);
  ...
  const getDashboardContent = () => {
    switch (user.role) {
      ...
      case 'HR':
        // HR User mode shows EmployeeDashboard, HR Admin mode shows WorkforceSummary
        return permissions.isHRUser ? <EmployeeDashboard /> : <WorkforceSummary />;
      case 'RMG':
        return <RMGDashboard />;
      case 'SUPER_ADMIN':
