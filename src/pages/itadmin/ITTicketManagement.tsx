...
export function ITTicketManagement() {
  ...
  const calculateStats = useCallback((ticketData: HelpdeskTicket[]) => {
    // Filter only IT tickets for accurate stats
    const itTickets = ticketData.filter((t) => t.highLevelCategory === 'IT');
    
    const newStats = {
      total: itTickets.length,
      inQueue: itTickets.filter((t) => t.status === 'In Queue').length,
      assigned: itTickets.filter(
      ...
      <div className="kpi-grid-5">
        <Card className="card-compact">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="kpi-label mb-1">Total Tickets</p>
        ...
        <Card className="card-compact">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="kpi-label mb-1">In Queue</p>
                <p className="kpi-value">
                  {stats.inQueue}
                </p>
              </div>
        ...
        <Card className="card-compact">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="kpi-label mb-1">Working On</p>
                <p className="kpi-value">
                  {stats.assigned}
                </p>
              </div>
        ...
        <Card className="card-compact">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="kpi-label mb-1">Completed</p>
                <p className="kpi-value">
                  {stats.completed}
                </p>
              </div>
        ...
        <Card className="card-compact">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="kpi-label mb-1">My Assigned</p>
                <p className="kpi-value">
                  {stats.myAssigned}
                </p>
              </div>
        ...
        {requestTypesWithCounts.map((type) => (
          <TabsContent key={type.id} value={type.id} className="mt-4">
            <SpecialistQueuePage
              ...
              onAssignToSelf={isITAdmin ? async () => {} : handleAssignToSelf}
              onAssignToSpecialist={isITAdmin ? async () => {} : handleAssignToSpecialist}
