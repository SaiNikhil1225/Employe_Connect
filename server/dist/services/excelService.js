...
class ExcelService {
    async generateTemplate(res) {
        ...
        const addDropdownValidation = (column, values, startRow = 2, endRow = 1000) => {
            const worksheet = dataSheet;
        ...
        // Add reference data
        const maxRows = Math.max(LEGAL_ENTITIES.length, LOCATIONS.length, DEPARTMENTS.length, BUSINESS_UNITS.length, WORKER_TYPES.length, HIRE_TYPES.length, LEAVE_PLANS.length, HOLIDAY_PLANS.length, GENDER_OPTIONS.length, BLOOD_GROUP_OPTIONS.length, MARITAL_STATUS.length, NATIONALITIES.length, PAYMENT_MODES.length, DIAL_CODES.length);
