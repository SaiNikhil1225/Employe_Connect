...
async function seedAllUsers() {
    try {
        ...
        await Employee_1.default.deleteMany({ email: { $in: emails } });
        console.log('✅ Cleared existing user records');
        // Hash passwords and insert users
        const users = await Promise.all(allUsersData.map(async (user) => {
        ...
        const createdUsers = await Employee_1.default.insertMany(users);
...
seedAllUsers();
//# sourceMappingURL=seedAllUsers.js.map