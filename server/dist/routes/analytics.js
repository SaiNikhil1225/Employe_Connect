...
router.get('/weekly-pattern', (0, permissions_1.checkModulePermission)({ module: 'RMG', action: 'view' }), async (req, res) => {
    try {
        const { startDate, endDate, category, priority, assignee } = req.query;
        ...
        const tickets = await HelpdeskTicket_1.default.find(filter).lean();
        ...
        tickets.forEach(ticket => {
            ...
            if (ticket.assignment?.assignedAt) {
                ...
                dailyStats[dayName].totalResponseTime += responseTime;
                dailyStats[dayName].responseCount++;
            }
        });
        // Calculate averages and format response
        const dailyPattern = Object.keys(dailyStats).map(day => ({
            day,
            created: dailyStats[day].created,
            resolved: dailyStats[day].resolved,
            avgResponseTime: dailyStats[day].responseCount > 0
                ? dailyStats[day].totalResponseTime / dailyStats[day].responseCount
                : 0,
            avgResolutionTime: dailyStats[day].resolutionCount > 0
                ? dailyStats[day].totalResolutionTime / dailyStats[day].resolutionCount
                : 0
        }));
        // Find busiest day
        const busiestDay = dailyPattern.reduce((max, current) => current.created > max.created ? current : max);
        // Find slowest day (highest avg resolution time)
        const slowestDay = dailyPattern.reduce((max, current) => current.avgResolutionTime > max.avgResolutionTime ? current : max);
        ...
        const totalResolved = tickets.filter(t => ['Closed', 'Completed', 'Confirmed', 'Auto-Closed'].includes(t.status)).length;
        ...
        const prevWeekTickets = await HelpdeskTicket_1.default.find(prevFilter).lean();
        // Calculate previous week metrics
        const prevTotalCreated = prevWeekTickets.length;
        const prevTotalResolved = prevWeekTickets.filter(t => ['Closed', 'Completed', 'Confirmed', 'Auto-Closed'].includes(t.status)).length;
...
router.get('/weekly-pattern/export', (0, permissions_1.checkModulePermission)({ module: 'RMG', action: 'view' }), async (req, res) => {
    try {
        const { startDate, endDate, category, priority, assignee } = req.query;
        // Default to last 7 days if no date range provided
        const end = endDate ? new Date(endDate) : new Date();
...
router.get('/monthly-statistics', (0, permissions_1.checkModulePermission)({ module: 'RMG', action: 'view' }), async (req, res) => {
    try {
        const { year, month, category, priority, assignee } = req.query;
        ...
        const currentMonthTickets = await HelpdeskTicket_1.default.find(buildFilter(startOfMonth, endOfMonth)).lean();
        // Fetch tickets for previous month
        const prevMonthTickets = await HelpdeskTicket_1.default.find(buildFilter(startOfPrevMonth, endOfPrevMonth)).lean();
        // Fetch tickets for same month last year
        const lastYearTickets = await HelpdeskTicket_1.default.find(buildFilter(startOfLastYearMonth, endOfLastYearMonth)).lean();
        ...
        const agentMetrics = Object.values(agentPerformance).map((agent) => ({
            name: agent.name,
