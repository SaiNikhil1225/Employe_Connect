...
router.post('/:id/rsvp', async (req, res) => {
    try {
        const { employeeId, userName, department, role, response, attendanceMode, guestCount, guests, dietaryRestrictions, specialRequirements, optionalNote, declineReason, device } = req.body;
        ...
        const existingRsvpIndex = rsvps.findIndex((r) => r.employeeId === employeeId);
        const timestamp = new Date();
        // Calculate view-to-RSVP time
        const viewDetail = event.viewDetails?.find((v) => v.employeeId === employeeId);
        ...
        event.attendingCount = rsvps.filter((r) => r.response === 'attending').length;
        event.declinedCount = rsvps.filter((r) => r.response === 'declined').length;
        event.maybeCount = rsvps.filter((r) => r.response === 'maybe').length;
        // Calculate attendance mode breakdown
        event.inPersonCount = rsvps.filter((r) => r.response === 'attending' && r.attendanceMode === 'in-person').length;
        event.virtualCount = rsvps.filter((r) => r.response === 'attending' && r.attendanceMode === 'virtual').length;
        // Track first RSVP
        ...
        if (event.viewDetails) {
            const viewIndex = event.viewDetails.findIndex((v) => v.employeeId === employeeId);
            if (viewIndex > -1) {
...
router.post('/:id/checkin', async (req, res) => {
    try {
        ...
        const rsvps = event.rsvps || [];
        const rsvpIndex = rsvps.findIndex((r) => r.employeeId === employeeId);
        if (rsvpIndex < 0) {
            // Walk-in (no RSVP)
        ...
        const attendedCount = rsvps.filter((r) => r.actualAttendance?.attended).length;
...
// Get RSVP list
router.get('/:id/analytics/rsvps', async (req, res) => {
    try {
        const event = await Event_1.default.findById(req.params.id);
        if (!event) {
            return res.status(404).json({ success: false, message: 'Event not found' });
        ...
        const rsvps = event.rsvps || [];
        // Sort by timestamp
        const sortedRsvps = rsvps.sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
        // Separate by response type
        const attending = sortedRsvps.filter((r) => r.response === 'attending');
        const declined = sortedRsvps.filter((r) => r.response === 'declined');
        const maybe = sortedRsvps.filter((r) => r.response === 'maybe');
        res.json({
            success: true,
            data: {
                total: rsvps.length,
...
router.get('/:id/analytics/views', async (req, res) => {
    try {
        ...
        const viewedAndResponded = views.filter((v) => v.hasResponded);
        const viewedButNotResponded = views.filter((v) => !v.hasResponded);
        res.json({
            success: true,
            data: {
                total: views.length,
