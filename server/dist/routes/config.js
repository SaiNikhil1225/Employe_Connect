...
router.get('/:type', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const configs = await ConfigMaster_1.default.find(query)
            .sort({ name: 1 })
            .lean();
        res.json({
            success: true,
...
router.get('/:type/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const config = await ConfigMaster_1.default.findOne({ _id: id, type });
        if (!config) {
...
router.post('/:type', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('RMG', 'SUPER_ADMIN'), async (req, res) => {
    try {
        const { type } = req.params;
        ...
        const configModel = ConfigMaster_1.default;
        ...
        // Create new config
        const config = new ConfigMaster_1.default({
            type,
            name: name.trim(),
...
router.put('/:type/:id', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('RMG', 'SUPER_ADMIN'), async (req, res) => {
    try {
        const { type, id } = req.params;
        const { name, description, status } = req.body;
        if (!validateType(type)) {
...
router.delete('/:type/:id', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('RMG', 'SUPER_ADMIN'), async (req, res) => {
    try {
        const { type, id } = req.params;
        if (!validateType(type)) {
            return res.status(400).json({
        ...
        const config = await ConfigMaster_1.default.findOneAndDelete({ _id: id, type });
        if (!config) {
            return res.status(404).json({
                success: false,
                message: 'Configuration not found'
...
router.patch('/:type/bulk-status', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('RMG', 'SUPER_ADMIN'), async (req, res) => {
    try {
        const { type } = req.params;
        ...
        const result = await ConfigMaster_1.default.updateMany({ _id: { $in: ids }, type }, {
            status,
            updatedBy: req.user?.employeeId || req.user?.email || ''
        });
        res.json({
