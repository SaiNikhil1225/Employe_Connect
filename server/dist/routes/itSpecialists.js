...
router.get('/', async (req, res) => {
    try {
        ...
        const specialists = await ITSpecialist_1.default.find(query).sort({ name: 1 });
...
router.get('/specialization/:specialization', async (req, res) => {
    try {
        ...
        const specialists = await ITSpecialist_1.default.find({
...
router.get('/:id', async (req, res) => {
    try {
        const specialist = await ITSpecialist_1.default.findById(req.params.id);
        if (!specialist) {
...
router.get('/employee/:employeeId', async (req, res) => {
    try {
        const specialist = await ITSpecialist_1.default.findOne({ employeeId: req.params.employeeId });
        if (!specialist) {
            return res.status(404).json({ success: false, message: 'Specialist not found' });
        }
        res.json({ success: true, data: specialist });
...
router.post('/', async (req, res) => {
    try {
        const specialist = new ITSpecialist_1.default(req.body);
...
router.put('/:id', async (req, res) => {
    try {
        const specialist = await ITSpecialist_1.default.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true, runValidators: true });
        if (!specialist) {
            return res.status(404).json({ success: false, message: 'Specialist not found' });
        }
        res.json({ success: true, data: specialist });
...
router.post('/:id/increment-tickets', async (req, res) => {
    try {
        const specialist = await ITSpecialist_1.default.findByIdAndUpdate(req.params.id, { $inc: { activeTicketCount: 1 } }, { new: true, runValidators: true });
        if (!specialist) {
            return res.status(404).json({ success: false, message: 'Specialist not found' });
        }
        res.json({ success: true, data: specialist });
...
router.post('/:id/decrement-tickets', async (req, res) => {
    try {
        const specialist = await ITSpecialist_1.default.findById(req.params.id);
        if (!specialist) {
            return res.status(404).json({ success: false, message: 'Specialist not found' });
...
router.delete('/:id', async (req, res) => {
    try {
        const specialist = await ITSpecialist_1.default.findByIdAndDelete(req.params.id);
        if (!specialist) {
            return res.status(404).json({ success: false, message: 'Specialist not found' });
        }
...
//# sourceMappingURL=itSpecialists.js.map