...
const OnboardingChecklist_1 = __importDefault(require("../models/OnboardingChecklist"));
const OffboardingChecklist_1 = __importDefault(require("../models/OffboardingChecklist"));
const CompanySettings_1 = __importDefault(require("../models/CompanySettings"));
const FLResource_1 = require("../models/FLResource");
 ...
 */
router.get('/my-module-permissions', auth_1.authenticateToken, async (req, res) => {
    try {
...
router.get('/utils/next-id', async (req, res) => {
    try {
        const hireType = req.query.workerType || req.query.hireType;
...
router.get('/validate/email', async (req, res) => {
    try {
        ...
        const existingEmployee = await Employee_1.default.findOne(query);
...
router.get('/validate/pan', async (req, res) => {
    try {
        ...
        const query = { panNumber: { $regex: new RegExp(`^${panNumber}$`, 'i') } };
        if (excludeId) {
            query._id = { $ne: excludeId };
        }
        const existingEmployee = await Employee_1.default.findOne(query);
...
router.get('/stats/workforce', async (_req, res) => {
    try {
        const now = new Date();
        ...
        // Get all employees
        const allEmployees = await Employee_1.default.find();
...
router.get('/allocations/details', async (_req, res) => {
    try {
        const today = new Date();
        const flResources = await FLResource_1.FLResource.find({
            status: 'Active',
        ...
        const projectObjectIds = [...new Set(flResources.map((r) => r.projectId?.toString()).filter(Boolean))];
        // Fetch matching projects once
        const projects = await Project_1.default.find({ _id: { $in: projectObjectIds } })
            .select('_id projectId projectName')
            .lean();
        const projectMap = new Map();
        projects.forEach((p) => projectMap.set(p._id.toString(), p));
        flResources.forEach((resource) => {
            if (!resource.employeeId)
                return;
            const empId = resource.employeeId;
            if (!detailsMap.has(empId))
...
router.get('/allocations/summary', async (_req, res) => {
    try {
        const today = new Date();
        ...
        flResources.forEach((resource) => {
            if (resource.employeeId && resource.utilizationPercentage) {
                ...
                allocationMap.set(resource.employeeId, currentAllocation + resource.utilizationPercentage);
            }
        });
...
function getEmployeeIdPrefix(hireType) {
    const prefixMap = {
...
router.post('/', validation_1.employeeValidation.create, async (req, res) => {
    try {
        ...
        if (employeeData.panNumber) {
            const existingPAN = await Employee_1.default.findOne({
        ...
        const existingEmail = await Employee_1.default.findOne({
        ...
        if (employeeData.reportingManagerId) {
            const reportingManager = await Employee_1.default.findOne({ employeeId: employeeData.reportingManagerId });
            if (reportingManager) {
        ...
        if (employeeData.dottedLineManagerId) {
            const dottedLineManager = await Employee_1.default.findOne({ employeeId: employeeData.dottedLineManagerId });
...
router.patch('/:id/inactive', async (req, res) => {
    try {
        ...
        const query = { $or: [{ id: req.params.id }, { employeeId: req.params.id }] };
        if (req.params.id.match(/^[0-9a-fA-F]{24}$/)) {
            query.$or.unshift({ _id: req.params.id });
        }
        const employee = await Employee_1.default.findOneAndUpdate(query, { status: 'inactive' }, { new: true });
        if (!employee) {
            return res.status(404).json({ success: false, message: 'Employee not found' });
        }
...
router.patch('/:id/activate', async (req, res) => {
    try {
        ...
        const employee = await Employee_1.default.findOneAndUpdate(query, { status: 'active' }, { new: true });
        if (!employee) {
            return res.status(404).json({ success: false, message: 'Employee not found' });
        }
        res.json({ success: true, data: employee });
...
// Update employee
router.put('/:id', auth_1.authenticateToken, validation_1.employeeValidation.update, async (req, res) => {
    try {
        ...
        if (updateData.reportingManagerId) {
            const reportingManager = await Employee_1.default.findOne({ employeeId: updateData.reportingManagerId });
            if (reportingManager) {
        ...
        if (updateData.dottedLineManagerId) {
            const dottedLineManager = await Employee_1.default.findOne({ employeeId: updateData.dottedLineManagerId });
            if (dottedLineManager) {
        ...
        const employee = await Employee_1.default.findOneAndUpdate(query, updateData, { new: true, runValidators: true });
        if (!employee) {
            return res.status(404).json({ success: false, message: 'Employee not found' });
        }
...
// Delete employee
router.delete('/:id', validation_1.employeeValidation.delete, async (req, res) => {
    try {
        // Build query to handle both MongoDB IDs and custom employee IDs
        const query = { $or: [{ id: req.params.id }, { employeeId: req.params.id }] };
        if (req.params.id.match(/^[0-9a-fA-F]{24}$/)) {
        ...
        const employee = await Employee_1.default.findOneAndDelete(query);
        if (!employee) {
            return res.status(404).json({ success: false, message: 'Employee not found' });
        }
...
router.post('/bulk', async (req, res) => {
    try {
        ...
        const result = await Employee_1.default.insertMany(processedEmployees);
...
router.get('/:id/profile', async (req, res) => {
    try {
        ...
        // Fetch related data
        const documents = await EmployeeDocument_1.default.find({
            employeeId: employee.employeeId,
            isActive: true
        });
        const onboarding = await OnboardingChecklist_1.default.findOne({
            employeeId: employee.employeeId
        });
        const offboarding = await OffboardingChecklist_1.default.findOne({
            employeeId: employee.employeeId
        });
        res.json({
            success: true,
...
router.patch('/:id/medical-info', auth_1.authenticateToken, async (req, res) => {
    try {
        // Authorization check: user can only edit their own profile or must be HR admin
        const requestingUser = req.user;
        const targetEmployeeId = req.params.id;
        ...
        const employee = await Employee_1.default.findOneAndUpdate({ $or: [{ id: req.params.id }, { employeeId: req.params.id }] }, { medicalInfo: req.body }, { new: true, runValidators: true });
        if (!employee) {
            return res.status(404).json({ success: false, message: 'Employee not found' });
        }
        res.json({ success: true, data: employee });
...
router.patch('/:id/emergency-contacts', auth_1.authenticateToken, async (req, res) => {
    try {
        // Authorization check: user can only edit their own profile or must be HR admin
        const requestingUser = req.user;
        const targetEmployeeId = req.params.id;
        ...
        const employee = await Employee_1.default.findOneAndUpdate({ $or: [{ id: req.params.id }, { employeeId: req.params.id }] }, { emergencyContacts: req.body.contacts }, { new: true, runValidators: true });
        if (!employee) {
            return res.status(404).json({ success: false, message: 'Employee not found' });
        }
        res.json({ success: true, data: employee });
...
router.patch('/:id/family-members', auth_1.authenticateToken, async (req, res) => {
    try {
        // Authorization check: user can only edit their own profile or must be HR admin
        const requestingUser = req.user;
        const targetEmployeeId = req.params.id;
        ...
        const employee = await Employee_1.default.findOneAndUpdate({ $or: [{ id: req.params.id }, { employeeId: req.params.id }] }, { familyMembers: req.body.members }, { new: true, runValidators: true });
        if (!employee) {
            return res.status(404).json({ success: false, message: 'Employee not found' });
        }
        res.json({ success: true, data: employee });
...
router.patch('/:id/contact-info', auth_1.authenticateToken, async (req, res) => {
    try {
        const requestingUser = req.user;
        const targetEmployeeId = req.params.id;
        ...
        if (req.body.alternateNumber !== undefined)
            updateData.alternateNumber = req.body.alternateNumber;
        if (req.body.personalEmail !== undefined)
            updateData.personalEmail = req.body.personalEmail;
        if (req.body.workEmail !== undefined)
            updateData.email = req.body.workEmail;
        if (req.body.residenceNumber !== undefined)
            updateData.residenceNumber = req.body.residenceNumber;
        const employee = await Employee_1.default.findOneAndUpdate({ $or: [{ id: req.params.id }, { employeeId: req.params.id }] }, updateData, { new: true, runValidators: true });
        if (!employee) {
            return res.status(404).json({ success: false, message: 'Employee not found' });
        }
        res.json({ success: true, data: employee });
...
router.patch('/:id/education-history', auth_1.authenticateToken, async (req, res) => {
    try {
        // Authorization check: user can only edit their own profile or must be HR admin
        const requestingUser = req.user;
        const targetEmployeeId = req.params.id;
        ...
        const employee = await Employee_1.default.findOneAndUpdate({ $or: [{ id: req.params.id }, { employeeId: req.params.id }] }, { educationHistory: req.body.educationHistory }, { new: true, runValidators: true });
        if (!employee) {
            return res.status(404).json({ success: false, message: 'Employee not found' });
        }
        res.json({ success: true, data: employee });
...
router.patch('/:id/banking-info', auth_1.authenticateToken, async (req, res) => {
    try {
        // Authorization check: user can only edit their own profile or must be HR admin
        const requestingUser = req.user;
        const targetEmployeeId = req.params.id;
...
// Return asset
router.patch('/:id/assets/:assetId/return', async (req, res) => {
    try {
        const employee = await Employee_1.default.findOne({
            $or: [{ id: req.params.id }, { employeeId: req.params.id }]
        });
        ...
        const asset = employee.assetsAssigned?.find((a) => a.assetId === req.params.assetId);
        if (!asset) {
...
router.patch('/:id/disable-login', async (req, res) => {
    try {
        const employee = await Employee_1.default.findOne({ employeeId: req.params.id });
        if (!employee) {
            return res.status(404).json({ success: false, message: 'Employee not found' });
        }
...
router.post('/:id/profile-picture', upload.single('profilePicture'), async (req, res) => {
    try {
        ...
        if (employee.profilePhoto) {
            const oldFilePath = path_1.default.join(__dirname, '../../uploads/profile-pictures', path_1.default.basename(employee.profilePhoto));
...
router.delete('/:id/profile-picture', async (req, res) => {
    try {
        ...
        if (employee.profilePhoto) {
            const filePath = path_1.default.join(__dirname, '../../uploads/profile-pictures', path_1.default.basename(employee.profilePhoto));
...
router.patch('/:id/enable-login', async (req, res) => {
    try {
        const employee = await Employee_1.default.findOne({ employeeId: req.params.id });
        if (!employee) {
            return res.status(404).json({ success: false, message: 'Employee not found' });
