...
router.get('/week/:employeeId/:weekStartDate', async (req, res) => {
    try {
        ...
        entries.forEach(entry => {
            ...
            const dayIndex = Math.floor((entryDate.getTime() - weekStartCopy.getTime()) / (1000 * 60 * 60 * 24));
        ...
        rowsWithMeta.forEach((row, idx) => {
            const hasRevisions = row.entryMeta?.some((meta) => meta?.approvalStatus === 'revision_requested');
            if (hasRevisions) {
...
router.post('/submit', async (req, res) => {
    try {
        ...
        const result = await TimesheetEntry_1.default.bulkWrite(bulkOps);
        // Calculate total hours
        const totalHours = (0, timesheetTransformers_1.calculateTotalHours)(entries);
...
router.post('/submit-daily', async (req, res) => {
    try {
        ...
        const targetDates = dates.map((dateStr) => {
        ...
        for (const date of targetDates) {
            ...
            for (const entry of dayEntries) {
                const [h, m] = entry.hours.split(':').map((v) => parseInt(v) || 0);
                totalMinutes += h * 60 + m;
            }
        ...
        for (let dayIndex = 0; dayIndex < 7; dayIndex++) {
            ...
            const shouldSubmitThisDay = targetDates.some(targetDate => targetDate.toISOString().split('T')[0] === currentDate.toISOString().split('T')[0]);
            if (!shouldSubmitThisDay)
                continue;
            for (const row of rows) {
                const hours = row.hours[dayIndex];
                if (hours && hours !== '00:00' && hours !== '0:00') {
...
router.put('/approve/:entryId', async (req, res) => {
    try {
        ...
        const entry = await TimesheetEntry_1.default.findByIdAndUpdate(entryId, {
...
router.get('/approvals', async (req, res) => {
    try {
        ...
        if (projectId && projectId !== 'all') {
            const managedProject = await Project_1.default.findOne({
                projectId,
        ...
        const entries = await TimesheetEntry_1.default.find(query).sort({ date: 1 });
        if (entries.length === 0) {
            return res.json(null);
        }
        const rows = (0, timesheetTransformers_1.dateEntriesToWeekRows)(weekStart, entries);
...
router.put('/approvals/approve-week', async (req, res) => {
    try {
        ...
        const weekEnd = new Date(weekStart);
        weekEnd.setDate(weekEnd.getDate() + 6);
        const result = await TimesheetEntry_1.default.updateMany({
            employeeId,
            projectId: projectId,
            date: { $gte: weekStart, $lte: weekEnd },
            status: 'submitted',
...
router.put('/approvals/bulk-approve-days', async (req, res) => {
    try {
        ...
        const bulkOps = dayIndices.map((dayIndex) => {
...
router.put('/approvals/revision-request', async (req, res) => {
    try {
        ...
        const bulkOps = reverts.map((item) => {
        ...
        for (const op of bulkOps) {
            ...
            const existingEntry = await TimesheetEntry_1.default.findOne(filter).select('date udaId approvalStatus status');
        ...
        const updatedEntries = await TimesheetEntry_1.default.find({
            employeeId,
            projectId: projectId,
        ...
        console.log(`[Revision Request] All entries for this week after update:`, updatedEntries.map(e => ({
...
//# sourceMappingURL=timesheetEntries.js.map