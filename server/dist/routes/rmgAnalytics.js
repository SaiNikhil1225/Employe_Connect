...
router.get('/resource-utilization', async (req, res) => {
    try {
        const { startDate, endDate, department, role, billable } = req.query;
        // For querying: use provided dates or no date filter
        let queryStart = undefined;
        let queryEnd = undefined;
        if (startDate)
        ...
        const responseEnd = endDate ? new Date(endDate) : new Date();
        ...
        const flResources = await FLResource_1.FLResource.find(flResourceFilter).lean();
        ...
        const employees = await Employee_1.default.find(employeeFilter).lean();
        // Fetch all active employees for global headcount and bench calculation
        const allActiveEmployeesQuery1 = await Employee_1.default.find({ status: 'active', isActive: true }).lean();
        const allActiveEmployeesQuery2 = await Employee_1.default.find({ status: 'active' }).lean();
        const allActiveEmployeesQuery3 = await Employee_1.default.countDocuments({ status: 'active', isActive: true });
        const allActiveEmployeesQuery4 = await Employee_1.default.countDocuments({});
        ...
        const projects = await Project_1.default.find({ _id: { $in: projectIds } }).lean();
        ...
        const totalUtilizationSum = Array.from(employeeUtilization.values())
            .reduce((sum, u) => sum + u.totalAllocation, 0);
        ...
        const totalBillableSum = Array.from(employeeUtilization.values())
            .reduce((sum, u) => sum + u.billableAllocation, 0);
        ...
        const totalNonBillableSum = Array.from(employeeUtilization.values())
            .reduce((sum, u) => sum + u.nonBillableAllocation, 0);
        ...
        const topPerformers = flResources.map(flResource => {
            ...
            if (flResource.projectId) {
                console.log('Looking for project:', flResource.projectId.toString(), 'Found:', project ? project.projectName : 'NOT FOUND');
            }
            return {
 ...
 * @queryparam {string} [department] - Filter by department
 */
router.get('/allocation-efficiency', async (req, res) => {
    try {
        ...
        const employees = await Employee_1.default.find(employeeFilter).lean();
        const projects = await Project_1.default.find({ status: 'active' }).lean();
        ...
        const overAllocated = Array.from(employeeAllocations.values())
            .filter((ea) => ea.totalAllocation > 100)
            .map((ea) => ({
        ...
        const underAllocated = Array.from(employeeAllocations.values())
            .filter((ea) => ea.totalAllocation < 50)
            .map((ea) => ({
            employeeId: ea.employee.employeeId,
            name: ea.employee.name,
            department: ea.employee.department,
        ...
        const optimalCount = Array.from(employeeAllocations.values())
            .filter((ea) => ea.totalAllocation >= 50 && ea.totalAllocation <= 100)
            .length;
        ...
        const totalAllocated = Array.from(employeeAllocations.values())
            .reduce((sum, ea) => sum + ea.totalAllocation, 0);
...
router.get('/cost-summary', async (req, res) => {
    try {
        ...
        const flResources = await FLResource_1.FLResource.find(flResourceFilter).lean();
        // Get projects
        const projectFilter = { status: { $in: ['active', 'on-hold', 'completed'] } };
        if (projectId) {
            projectFilter.projectId = projectId;
        }
        const projects = await Project_1.default.find(projectFilter).lean();
        const employeeCosts = new Map();
        employees.forEach(emp => {
            const empAllocs = flResources.filter(a => a.employeeId === emp.employeeId);
            ...
            const monthlySalary = emp.monthlySalary || 0; // eslint-disable-line @typescript-eslint/no-explicit-any
        ...
        projects.forEach(proj => {
            const projAllocs = flResources.filter((a) => a.projectId === proj.projectId && a.billable !== false);
...
router.get('/demand-forecast', async (req, res) => {
    try {
        ...
        const availableResources = employees.filter(emp => (currentUtilization.get(emp.employeeId) || 0) < 80).length;
        ...
        const demandByRole = Array.from(roleDemands.entries())
            .map(([role, demand]) => {
            const availableWithSkill = employees.filter(emp => emp.skills && emp.skills.includes(role) &&
                (currentUtilization.get(emp.employeeId) || 0) < 80).length;
            return {
                role,
                demand,
            ...
            data: {
                ...
                summary: {
                    ...
                    utilizationRate: employees.length > 0
                        ? Math.round((Array.from(currentUtilization.values()).reduce((s, u) => s + u, 0) / (employees.length * 100)) * 100 * 100) / 100
                        : 0
                },
                demandByRole,
                upcomingProjects,
...
//# sourceMappingURL=rmgAnalytics.js.map