...
const TrainingEnrollment_1 = require("../models/TrainingEnrollment");
const SkillGap_1 = require("../models/SkillGap");
const Employee_1 = __importDefault(require("../models/Employee"));
const router = express_1.default.Router();
// Helper function to get allowed employee IDs based on role
const getAllowedEmployeeIds = async (role, userId, department) => {
    if (role === 'HR' || role === 'SUPER_ADMIN' || role === 'RMG') {
        ...
        const manager = await Employee_1.default.findOne({ employeeId: userId });
        if (manager && manager.department) {
            const teamMembers = await Employee_1.default.find({
                department: manager.department,
                status: 'active'
            }).select('employeeId');
            return teamMembers.map((e) => e.employeeId);
        }
...
const applyStatCardFilters = (query, filters) => {
    // Time period filter
...
router.get('/', async (req, res) => {
    try {
        const { status, category, department, location, startDate, endDate, search } = req.query;
        const query = {};
        if (status)
            query.status = status;
        if (category)
            query.trainingCategory = category;
        if (department)
            query.targetDepartments = { $in: [department] };
        if (location)
            query.targetLocations = { $in: [location] };
        ...
        const trainings = await Training_1.Training.find(query).sort({ startDate: -1 });
...
router.get('/:id', async (req, res) => {
    try {
        const training = await Training_1.Training.findOne({ trainingId: req.params.id });
        if (!training) {
...
router.post('/', async (req, res) => {
    try {
        ...
        const count = await Training_1.Training.countDocuments();
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const training = await Training_1.Training.findOneAndUpdate({ trainingId: req.params.id }, updates, { new: true, runValidators: true });
        if (!training) {
            return res.status(404).json({ success: false, message: 'Training not found' });
        }
        res.json({ success: true, data: training });
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const enrollmentCount = await TrainingEnrollment_1.TrainingEnrollment.countDocuments({
        ...
        const training = await Training_1.Training.findOneAndDelete({ trainingId: req.params.id });
        if (!training) {
            return res.status(404).json({ success: false, message: 'Training not found' });
        }
...
router.get('/enrollments/all', async (req, res) => {
    try {
        const { trainingId, employeeId, department, location, completionStatus, certificationStatus, grade, employmentType, dateFrom, dateTo, role, userId } = req.query;
        const query = {};
        // Apply role-based filtering
        if (role && userId) {
            const allowedIds = await getAllowedEmployeeIds(role, userId);
            if (allowedIds !== null) {
        ...
        if (employeeId && !role)
            query.employeeId = employeeId; // Only apply if not using role-based filtering
        if (department)
        ...
        const enrollments = await TrainingEnrollment_1.TrainingEnrollment.find(query).sort({ enrollmentDate: -1 });
...
router.get('/:trainingId/enrollments', async (req, res) => {
    try {
        const enrollments = await TrainingEnrollment_1.TrainingEnrollment.find({
...
router.post('/enroll', async (req, res) => {
    try {
        ...
        const enrollments = [];
        const errors = [];
        let enrollmentCount = await TrainingEnrollment_1.TrainingEnrollment.countDocuments();
        for (const empId of employeeIds) {
            try {
                // Check if already enrolled
                const existingEnrollment = await TrainingEnrollment_1.TrainingEnrollment.findOne({
                ...
                const employee = await Employee_1.default.findById(empId);
                if (!employee) {
                ...
                // Create enrollment
                const enrollment = new TrainingEnrollment_1.TrainingEnrollment({
                    enrollmentId,
                    trainingId: training._id.toString(),
...
router.post('/:trainingId/enroll', async (req, res) => {
    try {
        const { trainingId } = req.params;
        const { employeeId, nominatedBy, approvedBy } = req.body;
        // Get training details
        const training = await Training_1.Training.findOne({ trainingId });
        if (!training) {
            return res.status(404).json({ success: false, message: 'Training not found' });
        }
        // Check if already enrolled
        ...
        // Generate enrollment ID
        const count = await TrainingEnrollment_1.TrainingEnrollment.countDocuments();
...
router.put('/enrollments/:enrollmentId', async (req, res) => {
    try {
        ...
        if (updates.completionStatus === 'Completed' && updates.certificationStatus === 'Pending') {
            ...
            const enrollment = await TrainingEnrollment_1.TrainingEnrollment.findOne({
                enrollmentId: req.params.enrollmentId
            });
            if (enrollment) {
                const training = await Training_1.Training.findOne({ trainingId: enrollment.trainingId });
        ...
        const enrollment = await TrainingEnrollment_1.TrainingEnrollment.findOneAndUpdate({ enrollmentId: req.params.enrollmentId }, updates, { new: true, runValidators: true });
        if (!enrollment) {
...
router.delete('/enrollments/:enrollmentId', async (req, res) => {
    try {
        ...
        await Training_1.Training.findOneAndUpdate({ trainingId: enrollment.trainingId }, { $inc: { currentEnrollments: -1 } });
...
router.get('/analytics/metrics', async (req, res) => {
    try {
        const { department, location, grade, employmentType, dateFrom, dateTo, role, userId, cardFilters } = req.query;
        ...
        if (cardFilters) {
            try {
                const filters = JSON.parse(cardFilters);
        ...
        if (department)
            enrollmentQuery.department = department;
        if (location)
            enrollmentQuery.location = location;
        if (grade)
            enrollmentQuery.grade = grade;
        if (employmentType)
            enrollmentQuery.employmentType = employmentType;
        if (dateFrom || dateTo) {
            const dateQuery = {};
            if (dateFrom)
                dateQuery.$gte = new Date(dateFrom);
        ...
        const enrollments = await TrainingEnrollment_1.TrainingEnrollment.find(enrollmentQuery);
        // Calculate metrics
        ...
        const certificationEligible = enrollments.filter(e => e.certificationStatus !== 'Not Applicable').length;
        const certificationCompleted = enrollments.filter(e => e.certificationStatus === 'Certified').length;
        const certificationCompletionRate = certificationEligible > 0
            ? (certificationCompleted / certificationEligible) * 100
            : 0;
        // Training completion rate
        const completedTrainings = enrollments.filter(e => e.completionStatus === 'Completed').length;
...
router.get('/analytics/hours-by-employee', async (req, res) => {
    try {
        ...
        if (employmentType)
            query.employmentType = employmentType;
        const enrollments = await TrainingEnrollment_1.TrainingEnrollment.find(query);
        // Group by employee
        const employeeData = {};
        enrollments.forEach(e => {
...
// ==================== SKILL GAP ANALYSIS ====================
// Get all skill gaps
router.get('/skill-gaps/all', async (req, res) => {
    try {
        ...
        if (employmentType)
            query.employmentType = employmentType;
        if (priority)
            query['skillGaps.priority'] = priority;
        const skillGaps = await SkillGap_1.SkillGap.find(query).sort({ overallGapScore: -1 });
...
router.get('/skill-gaps/employee/:employeeId', async (req, res) => {
    try {
        const skillGap = await SkillGap_1.SkillGap.findOne({ employeeId: req.params.employeeId });
        if (!skillGap) {
...
router.post('/skill-gaps', async (req, res) => {
    try {
        ...
        let totalGapScore = 0;
        let criticalCount = 0;
        const currentSkillsMap = new Map(skillGapData.currentSkills?.map((s) => [s.skillName, s]) || []);
        for (const requiredSkill of skillGapData.requiredSkills || []) {
            const currentSkill = currentSkillsMap.get(requiredSkill.skillName) || {};
            const currentScore = currentSkill.proficiencyScore || 0;
            ...
            if (gapScore > 0) {
                gaps.push({
                    ...
                    currentLevel: currentSkill.proficiencyLevel || 'None',
                ...
                if (requiredSkill.priority === 'Critical')
                    criticalCount++;
            }
        }
        ...
        const skillGap = await SkillGap_1.SkillGap.findOneAndUpdate({ employeeId }, skillGapData, { new: true, upsert: true, runValidators: true });
        res.json({ success: true, data: skillGap });
    }
    catch (error) {
...
router.get('/skill-gaps/analytics/summary', async (req, res) => {
    try {
        const { department, location, grade, employmentType } = req.query;
        const query = {};
        if (department)
        ...
        if (employmentType)
            query.employmentType = employmentType;
        const skillGaps = await SkillGap_1.SkillGap.find(query);
        ...
        skillGaps.forEach(sg => {
            sg.skillGaps.forEach(gap => {
                ...
                if (gap.priority === 'Critical')
                    skillGapMap[gap.skillName].criticalCount += 1;
                else if (gap.priority === 'High')
                    skillGapMap[gap.skillName].highCount += 1;
                else if (gap.priority === 'Medium')
                    skillGapMap[gap.skillName].mediumCount += 1;
                else if (gap.priority === 'Low')
                    skillGapMap[gap.skillName].lowCount += 1;
            });
        });
        // Calculate averages
        Object.values(skillGapMap).forEach((skill) => {
            skill.avgGapScore = skill.avgGapScore / skill.employeesWithGap;
        });
        // Sort by employees with gap (most common gaps)
        const sortedSkillGaps = Object.values(skillGapMap).sort((a, b) => b.employeesWithGap - a.employeesWithGap);
        res.json({
            success: true,
            data: {
