...
const HRRegionConfig_1 = require("../models/HRRegionConfig");
...
router.use((0, auth_1.authorizeRoles)('SUPER_ADMIN'));
// ===========================================
// DASHBOARD ROUTES
// ===========================================
/**
...
router.get('/dashboard/stats', asyncHandler(async (req, res) => {
    ...
    const openTickets = await HelpdeskTicket_1.default.countDocuments({
        status: { $nin: ['Completed', 'Cancelled', 'Closed'] }
    });
    const criticalTickets = await HelpdeskTicket_1.default.countDocuments({
        status: { $nin: ['Completed', 'Cancelled', 'Closed'] },
        urgency: 'Critical'
    });
    // Get pending approvals by level
    const pendingL1 = await HelpdeskTicket_1.default.countDocuments({ status: 'Pending L1 Approval' });
    const pendingL2 = await HelpdeskTicket_1.default.countDocuments({ status: 'Pending L2 Approval' });
    const pendingL3 = await HelpdeskTicket_1.default.countDocuments({ status: 'Pending L3 Approval' });
    // Get categories count
    const categoriesCount = await SubCategoryConfig_1.default.countDocuments({ isActive: true });
    const categoriesByType = await SubCategoryConfig_1.default.aggregate([
        { $match: { isActive: true } },
 ...
 */
router.get('/dashboard/health', asyncHandler(async (req, res) => {
...
router.get('/categories/:id', asyncHandler(async (req, res) => {
    const category = await SubCategoryConfig_1.default.findById(req.params.id);
    if (!category) {
...
router.post('/categories', asyncHandler(async (req, res) => {
    const { highLevelCategory, subCategory, requiresApproval, processingQueue, specialistQueue, order, isActive, approvalConfig } = req.body;
    // Check for duplicate
    const existing = await SubCategoryConfig_1.default.findOne({
        highLevelCategory,
        subCategory
    });
    if (existing) {
    ...
    const category = new SubCategoryConfig_1.default({
        highLevelCategory,
        subCategory,
 ...
 */
router.put('/categories/:id/approvers', asyncHandler(async (req, res) => {
...
router.get('/users/:id', asyncHandler(async (req, res) => {
    ...
    const approverAssignments = await SubCategoryConfig_1.default.find({
        $or: [
    ...
    const assignments = approverAssignments.map(cat => {
        const levels = [];
        const catObj = cat.toObject();
        if (catObj.approvalConfig?.l1?.approvers?.some((a) => a.employeeId === user.employeeId)) {
            levels.push('L1');
        }
        if (catObj.approvalConfig?.l2?.approvers?.some((a) => a.employeeId === user.employeeId)) {
            levels.push('L2');
        }
        if (catObj.approvalConfig?.l3?.approvers?.some((a) => a.employeeId === user.employeeId)) {
            levels.push('L3');
        }
        return {
...
router.post('/users', asyncHandler(async (req, res) => {
    ...
    if (employeeId) {
        const existingEmpId = await User_1.default.findOne({ employeeId });
        if (existingEmpId) {
            return res.status(409).json({
                success: false,
    ...
    const existingEmployee = await Employee_1.default.findOne({ email: email.toLowerCase() });
    if (!existingEmployee) {
...
router.put('/users/:id', asyncHandler(async (req, res) => {
    ...
    if (employeeId && employeeId !== user.employeeId) {
        const existingEmpId = await User_1.default.findOne({
            employeeId,
            _id: { $ne: req.params.id }
        });
        if (existingEmpId) {
...
router.get('/approvers', asyncHandler(async (req, res) => {
    const categories = await SubCategoryConfig_1.default.find({
        isActive: true,
        requiresApproval: true
    }).sort({ highLevelCategory: 1, order: 1 });
    // Get pending counts for each category
    const approversByCategory = await Promise.all(categories.map(async (cat) => {
        const catObj = cat.toObject();
        const pendingCounts = await HelpdeskTicket_1.default.aggregate([
            {
                $match: {
        ...
        pendingCounts.forEach((p) => {
            if (p._id === 'Pending L1 Approval')
                pending.l1 = p.count;
            if (p._id === 'Pending L2 Approval')
                pending.l2 = p.count;
            if (p._id === 'Pending L3 Approval')
                pending.l3 = p.count;
        });
        return {
            categoryId: cat._id.toString(),
 ...
 */
router.get('/approvers/stats', asyncHandler(async (req, res) => {
    ...
    pendingCounts.forEach((p) => {
        if (p._id === 'Pending L1 Approval')
            stats.l1Pending = p.count;
        if (p._id === 'Pending L2 Approval')
            stats.l2Pending = p.count;
        if (p._id === 'Pending L3 Approval')
            stats.l3Pending = p.count;
    });
    ...
    const l1Approvers = new Set();
    const l2Approvers = new Set();
    const l3Approvers = new Set();
    categories.forEach((cat) => {
        const catObj = cat.toObject();
        catObj.approvalConfig?.l1?.approvers?.forEach((a) => l1Approvers.add(a.employeeId));
        catObj.approvalConfig?.l2?.approvers?.forEach((a) => l2Approvers.add(a.employeeId));
        catObj.approvalConfig?.l3?.approvers?.forEach((a) => l3Approvers.add(a.employeeId));
    });
    // Get total approvals count
    const totalApprovals = await HelpdeskTicket_1.default.countDocuments({
 ...
 */
router.get('/approvers/list', asyncHandler(async (req, res) => {
    const categories = await SubCategoryConfig_1.default.find({
        isActive: true,
        requiresApproval: true
    });
    ...
    categories.forEach((cat) => {
        ...
        catObj.approvalConfig?.l1?.approvers?.forEach((a) => {
        ...
        catObj.approvalConfig?.l2?.approvers?.forEach((a) => {
            if (!approverMap.has(a.employeeId)) {
                approverMap.set(a.employeeId, {
                    employeeId: a.employeeId,
                    name: a.name,
        ...
        catObj.approvalConfig?.l3?.approvers?.forEach((a) => {
            if (!approverMap.has(a.employeeId)) {
                approverMap.set(a.employeeId, {
                    employeeId: a.employeeId,
                    name: a.name,
...
router.get('/permissions', asyncHandler(async (req, res) => {
    ...
    const permissionsWithEmployees = await Promise.all(permissions.map(async (permission) => {
        const employee = await Employee_1.default.findOne({ employeeId: permission.employeeId });
        return {
            ...permission.toObject(),
            employee: employee ? {
                name: employee.name,
 ...
 */
router.get('/permissions/:employeeId', asyncHandler(async (req, res) => {
    const { employeeId } = req.params;
    const permissions = await ModulePermission_1.ModulePermission.find({ employeeId });
    res.json({
        success: true,
        data: permissions,
    });
...
router.post('/permissions', asyncHandler(async (req, res) => {
    ...
    // Upsert permission
    const permission = await ModulePermission_1.ModulePermission.findOneAndUpdate({ employeeId, module }, {
        employeeId,
        module,
        enabled,
        isAdmin,
 ...
 */
router.put('/permissions/:id', asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { enabled, isAdmin, permissions } = req.body;
    const permission = await ModulePermission_1.ModulePermission.findByIdAndUpdate(id, { enabled, isAdmin, permissions }, { new: true });
    if (!permission) {
        return res.status(404).json({
            success: false,
 ...
 */
router.delete('/permissions/:id', asyncHandler(async (req, res) => {
    const { id } = req.params;
    const permission = await ModulePermission_1.ModulePermission.findByIdAndDelete(id);
    if (!permission) {
        return res.status(404).json({
            success: false,
            message: 'Permission not found',
 ...
 */
router.get('/leave-policies', asyncHandler(async (req, res) => {
    const policies = await LeavePolicy_1.LeavePolicy.find({ isActive: true });
    res.json({
        success: true,
        data: policies,
    });
 ...
 * Create leave policy
 */
router.post('/leave-policy', asyncHandler(async (req, res) => {
    const policyData = req.body;
    const policy = await LeavePolicy_1.LeavePolicy.findOneAndUpdate({ leaveType: policyData.leaveType, country: policyData.country }, { ...policyData, isActive: true }, { upsert: true, new: true });
    res.json({
        success: true,
 ...
 * Update leave policy
 */
router.put('/leave-policy/:id', asyncHandler(async (req, res) => {
    const { id } = req.params;
    const policyData = req.body;
    const policy = await LeavePolicy_1.LeavePolicy.findByIdAndUpdate(id, policyData, {
        new: true,
    });
    if (!policy) {
        return res.status(404).json({
 ...
 * Delete leave policy
 */
router.delete('/leave-policy/:id', asyncHandler(async (req, res) => {
    const { id } = req.params;
    const policy = await LeavePolicy_1.LeavePolicy.findByIdAndUpdate(id, { isActive: false }, { new: true });
    if (!policy) {
        return res.status(404).json({
            success: false,
            message: 'Leave policy not found',
 ...
 */
router.get('/hr-regions', asyncHandler(async (req, res) => {
    const configs = await HRRegionConfig_1.HRRegionConfig.find({ isActive: true });
    res.json({
        success: true,
        data: configs,
    });
 ...
 */
router.get('/hr-region/:region', asyncHandler(async (req, res) => {
    const { region } = req.params;
    const config = await HRRegionConfig_1.HRRegionConfig.findOne({ region, isActive: true });
    if (!config) {
        return res.status(404).json({
            success: false,
 ...
 */
router.post('/hr-region', asyncHandler(async (req, res) => {
    const { region, fields, departments, designations } = req.body;
    const config = await HRRegionConfig_1.HRRegionConfig.findOneAndUpdate({ region }, { region, fields, departments, designations, isActive: true }, { upsert: true, new: true });
    res.json({
        success: true,
 ...
 */
router.put('/hr-region/:id', asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { fields, departments, designations } = req.body;
    const config = await HRRegionConfig_1.HRRegionConfig.findByIdAndUpdate(id, { fields, departments, designations }, { new: true });
    if (!config) {
        return res.status(404).json({
            success: false,
            message: 'Region config not found',
