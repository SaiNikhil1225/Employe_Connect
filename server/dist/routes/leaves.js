...
router.get('/', async (req, res) => {
    try {
        ...
        if (status)
            query.status = status;
        const leaves = await Leave_1.default.find(query).sort({ appliedOn: -1 });
...
router.get('/pending', async (_req, res) => {
    try {
        const leaves = await Leave_1.default.find({ status: 'pending' }).sort({ appliedOn: -1 });
        res.json({ success: true, data: leaves });
    }
    catch (error) {
...
router.get('/balance/:userId', async (req, res) => {
    try {
        ...
        let leaveBalance = await LeaveBalance_1.default.findOne({
            employeeId: req.params.userId,
            year: currentYear
        });
        // If no balance found, create default one
        if (!leaveBalance) {
            leaveBalance = new LeaveBalance_1.default({
                employeeId: req.params.userId,
                year: currentYear
            });
...
router.post('/', validation_1.leaveValidation.create, async (req, res) => {
    try {
        ...
        const leave = new Leave_1.default(req.body);
        await leave.save();
...
router.put('/:id', validation_1.leaveValidation.update, async (req, res) => {
    try {
        const leave = await Leave_1.default.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
        if (!leave) {
            return res.status(404).json({ success: false, message: 'Leave not found' });
        }
        res.json({ success: true, data: leave });
...
router.delete('/:id', async (req, res) => {
    try {
        const leave = await Leave_1.default.findByIdAndDelete(req.params.id);
        if (!leave) {
            return res.status(404).json({ success: false, message: 'Leave not found' });
        }
