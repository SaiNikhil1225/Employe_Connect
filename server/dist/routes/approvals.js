...
const checkApproverRole = (requiredRole) => {
    return async (req, res, next) => {
        try {
            ...
            const user = await Employee_1.default.findById(approverId);
            if (!user) {
...
// L1 Approval Handler
router.post('/l1/:ticketId', checkApproverRole('L1_APPROVER'), async (req, res) => {
    try {
        ...
        const existingApproval = ticket.approverHistory?.find((h) => h.level === 'L1' && h.approverId === approverId);
        if (existingApproval) {
            return res.status(400).json({
                success: false,
        ...
        await notificationService_1.default.notifyL1Approval({
...
// L2 Approval Handler
router.post('/l2/:ticketId', checkApproverRole('L2_APPROVER'), async (req, res) => {
    try {
        const { ticketId } = req.params;
        const { approverId, approverName, approverEmail, status, comments } = req.body;
        const ticket = await HelpdeskTicket_1.default.findById(ticketId);
        ...
        // Check for duplicate approval
        const existingApproval = ticket.approverHistory?.find((h) => h.level === 'L2' && h.approverId === approverId);
        if (existingApproval) {
            return res.status(400).json({
                success: false,
        ...
        await notificationService_1.default.notifyL2Approval({
            ticketNumber: ticket.ticketNumber,
            userId: ticket.userId,
            userName: ticket.userName,
            subject: ticket.subject,
...
router.post('/l3/:ticketId', checkApproverRole('L3_APPROVER'), async (req, res) => {
    try {
        const { ticketId } = req.params;
        const { approverId, approverName, approverEmail, status, comments } = req.body;
        const ticket = await HelpdeskTicket_1.default.findById(ticketId);
        ...
        // Check for duplicate approval
        const existingApproval = ticket.approverHistory?.find((h) => h.level === 'L3' && h.approverId === approverId);
        if (existingApproval) {
            return res.status(400).json({
                success: false,
        ...
        await notificationService_1.default.notifyL3Approval({
            ticketNumber: ticket.ticketNumber,
            userId: ticket.userId,
            userName: ticket.userName,
            subject: ticket.subject,
...
router.get('/pending/:approverId', async (req, res) => {
    try {
        ...
        let approverLevel = '';
        if (user.role === 'L1_APPROVER')
            approverLevel = 'L1';
        else if (user.role === 'L2_APPROVER')
            approverLevel = 'L2';
        else if (user.role === 'L3_APPROVER')
            approverLevel = 'L3';
        else {
            return res.status(403).json({
                success: false,
        ...
        const allApprovalTickets = await HelpdeskTicket_1.default.find({
        ...
        const ticketsWithMetadata = allApprovalTickets.map(ticket => {
            ...
            console.log('  ', statusIcon, ticket.ticketNumber, '- Current Level:', currentLevel, '- User Level:', approverLevel, '- Status:', ticket.status);
            return ticketObj;
        });
...
router.get('/all/:approverId', async (req, res) => {
    try {
        const { approverId } = req.params;
        const user = await Employee_1.default.findById(approverId);
        if (!user) {
        ...
        const allTickets = await HelpdeskTicket_1.default.find({
            requiresApproval: true,
        }).sort({ createdAt: -1 });
