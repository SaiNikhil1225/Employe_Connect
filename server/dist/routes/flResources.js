...
router.get('/', async (req, res) => {
    try {
        ...
        if (financialLineId) {
            ...
            if (mongoose_1.default.Types.ObjectId.isValid(financialLineId)) {
                query.financialLineId = new mongoose_1.default.Types.ObjectId(financialLineId);
            }
            else {
        ...
        const resources = await FLResource_1.FLResource.find(query)
...
router.get('/employee/:employeeId', async (req, res) => {
    try {
        const resources = await FLResource_1.FLResource.find({
...
router.get('/project/:projectId', async (req, res) => {
    try {
        ...
        if (mongoose_1.default.Types.ObjectId.isValid(projectId)) {
            ...
            projectObjectId = new mongoose_1.default.Types.ObjectId(projectId);
            ...
            const project = await Project_1.default.findOne({ projectId: projectId });
            if (!project) {
...
function convertTimeStringToHours(timeString) {
    if (!timeString || timeString === '0' || timeString === '0:0')
        return 0;
    ...
    try {
        ...
        if (projectId) {
            if (mongoose_1.default.Types.ObjectId.isValid(projectId)) {
                ...
                try {
                    const project = await Project_1.default.findById(projectObjectId);
                    if (project) {
                ...
                projectStringId = projectId;
                const project = await Project_1.default.findOne({ projectId: projectStringId });
                if (project) {
                    projectObjectId = project._id;
                    query.projectId = projectObjectId;
        ...
        const resourcesWithHours = await Promise.all(resources.map(async (resource) => {
            const resourceObj = resource.toObject();
            ...
            if (resource.utilizationPercentage && resource.requestedFromDate && resource.requestedToDate) {
                ...
                const effectiveStartDate = startDate
                    ? new Date(Math.max(allocationStartDate.getTime(), new Date(startDate).getTime()))
                    : allocationStartDate;
                const effectiveEndDate = endDate
                    ? new Date(Math.min(allocationEndDate.getTime(), new Date(endDate).getTime()))
                    : allocationEndDate;
                ...
                if (startDate || endDate || resource.requestedFromDate || resource.requestedToDate) {
                    ...
                    if (timesheetStartDate || timesheetEndDate) {
                        ...
                        if (timesheetStartDate) {
                            timesheetQuery.date.$gte = timesheetStartDate;
                        }
                        if (timesheetEndDate) {
                            timesheetQuery.date.$lte = timesheetEndDate;
                        }
                    }
                }
                const timesheetEntries = await TimesheetEntry_1.default.find(timesheetQuery);
                ...
                if (startDate || endDate || resource.requestedFromDate || resource.requestedToDate) {
                    ...
                    if (approvedStartDate || approvedEndDate) {
                        ...
                        if (approvedStartDate) {
                            approvedQuery.date.$gte = approvedStartDate;
                        }
                        if (approvedEndDate) {
                            approvedQuery.date.$lte = approvedEndDate;
                        }
                    }
                }
                const approvedEntries = await TimesheetEntry_1.default.find(approvedQuery);
...
router.get('/:id', async (req, res) => {
    try {
        const resource = await FLResource_1.FLResource.findById(req.params.id);
        if (!resource) {
...
router.post('/', async (req, res) => {
    try {
        ...
        const newResource = new FLResource_1.FLResource(resourceData);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const resource = await FLResource_1.FLResource.findByIdAndUpdate(req.params.id, updates, { new: true, runValidators: true });
        if (!resource) {
            return res.status(404).json({ message: 'Resource not found' });
        }
        res.json(resource);
...
router.delete('/:id', async (req, res) => {
    try {
        const resource = await FLResource_1.FLResource.findByIdAndDelete(req.params.id);
        if (!resource) {
            return res.status(404).json({ message: 'Resource not found' });
        }
...
router.get('/by-fl/:flId', async (req, res) => {
    try {
        const resources = await FLResource_1.FLResource.find({ financialLineId: req.params.flId }).sort({ createdAt: -1 });
        res.json(resources);
    }
    catch (error) {
        console.error('Error fetching FL resources:', error);
...
router.get('/by-project/:projectId', async (req, res) => {
    try {
        const resources = await FLResource_1.FLResource.find({ projectId: req.params.projectId }).sort({ createdAt: -1 });
        res.json(resources);
    }
    catch (error) {
        console.error('Error fetching project resources:', error);
...
//# sourceMappingURL=flResources.js.map