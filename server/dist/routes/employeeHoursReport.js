...
function convertHoursToDecimal(timeString) {
...
router.get('/', async (req, res) => {
    try {
        const { role, employeeId, managerId, month, startDate, endDate, projectId, department, } = req.query;
        ...
        if (role === 'EMPLOYEE') {
            ...
            const managedProjects = await Project_1.default.find({
                $or: [
            ...
            employeeIds = [...new Set(flResources.map((r) => r.employeeId).filter((id) => !!id))];
            ...
            if (projectId) {
                ...
                const project = await Project_1.default.findOne({ projectId });
                if (project) {
                    console.log('✅ Project found:', project.projectName);
                    const flResources = await FLResource_1.FLResource.find({ projectId: project._id });
                ...
                const allEmployees = await Employee_1.default.find({}).select('employeeId').limit(1000); // Limit to prevent memory issues
        ...
        if (role === 'RMG' && department && department !== 'all') {
            ...
            const deptEmployees = await Employee_1.default.find({ department }).select('employeeId');
        ...
        for (const empId of employeeIds) {
            try {
                ...
                const employee = await Employee_1.default.findOne({ employeeId: empId });
                if (!employee) {
                    console.log(`⚠️ Employee not found: ${empId}`);
                    continue;
                }
                // Get allocations
                let allocationQuery = { employeeId: empId };
                ...
                const allocations = await FLResource_1.FLResource.find(allocationQuery);
                ...
                let timesheetQuery = { employeeId: empId };
                ...
                const entries = await TimesheetEntry_1.default.find(timesheetQuery);
...
//# sourceMappingURL=employeeHoursReport.js.map