...
router.post('/:id/vote', async (req: Request, res: Response) => {
  try {
    ...
    const hasVoted = poll.options.some((option: any) => 
      option.voters.some((v: any) => v.employeeId === employeeId)
    );
    ...
    const viewDetail = poll.viewDetails?.find((v: any) => v.employeeId === employeeId);
    ...
    // Update view details
    if (poll.viewDetails) {
      const viewIndex = poll.viewDetails.findIndex((v: any) => v.employeeId === employeeId);
      if (viewIndex > -1) {
...
router.get('/:id/analytics', async (req: Request, res: Response) => {
  try {
    ...
    // Vote distribution
    const voteDistribution = poll.options.map((option: any) => ({
      id: option.id,
      text: option.text,
      votes: option.votes,
...
router.get('/:id/analytics/voters', async (req: Request, res: Response) => {
  try {
    const poll = await Poll.findById(req.params.id);
    if (!poll) {
      return res.status(404).json({ success: false, message: 'Poll not found' });
...
router.get('/:id/analytics/views', async (req: Request, res: Response) => {
  try {
    ...
    const viewedAndVoted = views.filter((v: any) => v.hasVoted);
    const viewedButNotVoted = views.filter((v: any) => !v.hasVoted);
...
router.get('/:id/analytics/timeline', async (req: Request, res: Response) => {
  try {
    ...
    // Add comments
    (poll.comments || []).forEach((comment: any) => {
      timeline.push({
        type: 'comment',
