...
router.post('/:id/like', async (req: Request, res: Response) => {
  try {
    ...
    const existingReactionIndex = reactions.findIndex((r: any) => r.employeeId === userId && r.emoji === '❤️');
    ...
    if (userIndex > -1 || existingReactionIndex > -1) {
      ...
      if (announcement.viewDetails) {
        const viewIndex = announcement.viewDetails.findIndex((v: any) => v.employeeId === userId);
        if (viewIndex > -1) {
...
router.post('/:id/comment', async (req: Request, res: Response) => {
  try {
    ...
    // Update view engagement status
    if (announcement.viewDetails) {
      const viewIndex = announcement.viewDetails.findIndex((v: any) => v.employeeId === employeeId);
      if (viewIndex > -1) {
        announcement.viewDetails[viewIndex].hasEngaged = true;
      }
    }
...
router.post('/:id/reaction', async (req: Request, res: Response) => {
  try {
    ...
    const reactions = announcement.reactions || [];
    const existingReactionIndex = reactions.findIndex((r: any) => r.employeeId === userIdToUse || r.oderId === userIdToUse);
    const timestamp = new Date();
    ...
    // Update view engagement status
    if (announcement.viewDetails) {
      const viewIndex = announcement.viewDetails.findIndex((v: any) => v.employeeId === userIdToUse);
      if (viewIndex > -1) {
        announcement.viewDetails[viewIndex].hasEngaged = true;
      }
    }
...
router.get('/:id/analytics', async (req: Request, res: Response) => {
  try {
    ...
    // Reaction breakdown
    const reactionBreakdown: any = {};
    (announcement.reactions || []).forEach((r: any) => {
      reactionBreakdown[r.label] = (reactionBreakdown[r.label] || 0) + 1;
    });

    // Department breakdown
    const departmentViewsArray: any[] = [];
    const departmentViews: any = {};
    (announcement.viewDetails || []).forEach((v: any) => {
      if (v.department) {
    ...
    // Get unique viewers
    const uniqueViewers = new Set((announcement.viewDetails || []).map((v: any) => v.employeeId));
    ...
    const analytics = {
      ...
      comments: {
        total: commentsCount,
        uniqueCommenters: new Set((announcement.comments || []).map((c: any) => c.employeeId)).size
      },
      shares: sharesCount,
...
router.get('/:id/analytics/reactions', async (req: Request, res: Response) => {
  try {
    const announcement = await Announcement.findById(req.params.id);
    if (!announcement) {
      return res.status(404).json({ success: false, message: 'Announcement not found' });
    ...
    const sortedReactions = reactions.sort((a: any, b: any) => 
...
router.get('/:id/analytics/comments', async (req: Request, res: Response) => {
  try {
    const announcement = await Announcement.findById(req.params.id);
    if (!announcement) {
      return res.status(404).json({ success: false, message: 'Announcement not found' });
    ...
    // Calculate stats
    const uniqueCommenters = new Set(comments.map((c: any) => c.employeeId)).size;
    ...
    res.json({ 
      ...
      data: {
        ...
        uniqueCommenters,
        avgCommentsPerPerson: parseFloat(avgCommentsPerPerson as string),
...
router.get('/:id/analytics/views', async (req: Request, res: Response) => {
  try {
    ...
    // Sort by timestamp
    const sortedViews = views.sort((a: any, b: any) => 
      new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()
    );
...
router.get('/:id/analytics/timeline', async (req: Request, res: Response) => {
  try {
    ...
    // Add comments
    (announcement.comments || []).forEach((c: any) => {
      timeline.push({
        type: 'comment',
