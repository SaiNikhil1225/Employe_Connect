 ...
 */
function calculateExpectedWorkingDays(startDate: Date, endDate: Date): number {
    const daysInRange = eachDayOfInterval({ start: startDate, end: endDate });
    const workingDays = daysInRange.filter(day => !isWeekend(day));
 ...
 */
export async function calculateEmployeeKPIs(employeeId: string, dateRange: DateRange) {
    ...
    const presentDays = logs.filter(log =>
        log.status === 'present' || log.status === 'wfh' || log.status === 'late' || log.status === 'half-day'
    ).length;
    ...
    const earlyLogoutFrequency = logs.filter(log => {
        ...
        if (log.isEarlyLogout) return true;
 ...
 */
export async function calculateTeamKPIs(filters: { department?: string; employeeIds?: string[] }, dateRange: DateRange) {
    const { startDate, endDate } = dateRange;
    ...
    if (employeeIds) employeeQuery.employeeId = { $in: employeeIds };

    const teamEmployees = await Employee.find(employeeQuery);
 ...
 */
export function canRegularizeAttendance(log: any): { canRegularize: boolean; reason: string } {
 ...
 */
export async function getEnhancedAttendanceLogs(filters: any, dateRange: DateRange) {
    const { startDate, endDate } = dateRange;
    ...
    const enhancedLogs = logs.map(log => {
        ...
        return {
            ...
            shiftTiming: log.shift ? `${log.shift}: ${SHIFTS[log.shift as keyof typeof SHIFTS].startTime} - ${SHIFTS[log.shift as keyof typeof SHIFTS].endTime} IST` : undefined
        };
    });
